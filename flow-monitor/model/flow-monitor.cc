...
FlowMonitor::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FlowMonitor")
    ...
    .AddAttribute ("FlowInterruptionsMinTime", ("The minimum inter-arrival time that is considered a flow interruption."),
                   ...
                   MakeTimeChecker ())
  .AddAttribute ("EwmaTimeConstant", ("EWMA time constant in nanoseconds, amount of time for the smoothed response of a unit set function to reach 63% of original."),
                 DoubleValue (80000),
                 MakeDoubleAccessor (&FlowMonitor::m_ewmaTimeConstant),
                 MakeDoubleChecker <double> ());
  ...
  if (stats.rxPackets == 1)
  ...
  else
    {
      ...
      if (stats.rxPackets == 2 || instantRate < 10) {
        ...
        double ewma_time_constant = m_ewmaTimeConstant; // units are nanoseconds, default is 80000ns
...
double FlowMonitor::getEwmaRate(FlowId flow_id) const {
  const auto& it = m_flowStats.find(flow_id);
  if (it != m_flowStats.end()) return it->second.rxEwmaRate;
  else return 0;
}
